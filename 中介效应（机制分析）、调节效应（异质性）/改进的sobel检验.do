// 配合视频食用：https://www.bilibili.com/list/watchlater?bvid=BV1Gs4y1M7Y8&oid=953737206
*- New
...
`quietly' {
 ...
 `prefix' reghdfe `varlist' `iv' `cv' if `touse', absorb(branch time)
 *`prefix' xtreg `varlist' `iv' `cv' i.time if `touse', fe
 ...
 `prefix' reghdfe `mv' `iv' `cv' if `touse', absorb(branch time)
 *`prefix' xtreg `mv' `iv' `cv' i.time if `touse', fe
 ...
 `prefix' reghdfe `varlist' `mv' `iv' `cv' if `touse', absorb(branch time)
